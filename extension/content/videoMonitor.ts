...
interface VideoState {
  ...
  title?: string;
  duration?: number;
  videoId?: string; // Added to track unique video IDs
  platform?: string; // Added to identify the platform
  pageTitle?: string; // Added to track page title changes
  metaData?: { [key: string]: string }; // Added to track metadata changes
}

// Track states and playback status
const videoStates = new WeakMap<HTMLVideoElement, VideoState>();
const knownIframes = new Map<string, string>(); // Map iframe element ID to its current src
let changeCount = 0;
let hasInitialPlayback = false;
let allMediaElements: HTMLVideoElement[] = [];
...
let lastMetaData: { [key: string]: string } = {}; // Track metadata
let forceScanInterval: number | null = null; // For aggressive scanning
  ...
  if (message.type === "AUDIO_STATUS_CHANGED") {
    ...
    if (isTabAudible && !wasAudible) {
      ...
      if (allMediaElements.length === 0) {
        debugLog(
          "Audio detected but no visible videos - treating as potential video change"
        );
...
function scanForHiddenPlayers() {
  ...
  document.querySelectorAll("iframe").forEach((iframe) => {
    try {
      handleIframeChange(iframe as HTMLIFrameElement);
    } catch (e) {
  ...
  const potentialPlayers = document.querySelectorAll(
    '[class*="player"], [class*="video"], [class*="media"], [id*="player"], [id*="video"], [id*="media"], video, audio, [role="application"]'
  );
...
const platformPatterns = {
  youtube: {
    videoIdRegex:
      /(?:v=|\/embed\/|\/v\/|youtu\.be\/|\/watch\?v=|\/watch\?.+&v=)([^#&?\/]+)/i,
...
function debugLog(message: string, data?: any) {
  const logMessage = `[VideoMonitor] ${message} ${
...
function extractMetadata(): { [key: string]: string } {
...
function hasMetadataChanged(
  oldMeta: { [key: string]: string },
  newMeta: { [key: string]: string }
): boolean {
  ...
  const jsonLdKeys = Object.keys(newMeta).filter((k) =>
    k.startsWith("jsonld_")
  );
...
function hasUrlChanged(oldUrl: string, newUrl: string): boolean {
  ...
  try {
    ...
    if (oldUrlObj.hostname !== newUrlObj.hostname) return true;

    // Check for path changes that indicate a new video
    if (oldUrlObj.pathname !== newUrlObj.pathname) return true;
    ...
    if (oldVideoId && newVideoId && oldVideoId !== newVideoId) return true;
...
function extractVideoId(url: string): {
  ...
  if (!url) return { videoId: null, platform: null };
  ...
  try {
    return {
      videoId: url.split("?")[0], // Remove query parameters
...
function getPlatformSpecificTitle(): string {
...
function hasVideoChanged(
  ...
  previousState: VideoState | undefined
): boolean {
  if (!previousState) return true; // New video
  ...
  if (
    Math.abs((currentState.duration || 0) - (previousState.duration || 0)) > 1
  ) {
  ...
  if (currentState.isPlaying || isTabAudible) {
    ...
    if (
      currentState.pageTitle !== previousState.pageTitle &&
...
function handleVideoChange(video: HTMLVideoElement) {
...
function handleIframeChange(iframe: HTMLIFrameElement) {
...
function setupVideoMonitoring(video: HTMLVideoElement) {
...
function cleanupVideoMonitoring(video: HTMLVideoElement) {
...
function monitorElements() {
  ...
  const videos = allElements.filter(
    (el) => el.tagName === "VIDEO"
  ) as HTMLVideoElement[];
  ...
  const iframes = allElements.filter(
  ...
  if (
    previousCount !== allMediaElements.length &&
...
function getShadowElements(root: Document | ShadowRoot): Element[] {
...
function observeShadowRoot(shadowRoot: ShadowRoot) {
  ...
  videos.forEach((video) => setupVideoMonitoring(video as HTMLVideoElement));

  const iframes = shadowRoot.querySelectorAll("iframe");
  iframes.forEach((iframe) => handleIframeChange(iframe as HTMLIFrameElement));
