...
let temporaryAllowances: {
  [key: string]: { expiry: number; purpose: string };
} = {}; // Store temporary allowances with expiry timestamps and purpose
...
function checkWebsiteCategory(
  ...
  try {
    ...
    if (
      websiteCategories.fun.some(
        (site) => new URL(site.url).hostname === domain
      )
    ) {
      return "fun";
    }
    if (
      websiteCategories.funAndWork.some(
        (site) => new URL(site.url).hostname === domain
      )
    ) {
      return "funAndWork";
    ...
    if (
      websiteCategories.socialMedia.some(
        (site) => new URL(site.url).hostname === domain
      )
    ) {
      return "socialMedia";
...
function isTemporarilyAllowed(url: string): boolean {
  try {
