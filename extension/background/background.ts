...
interface TimerSettings {
  ...
  maxVideoChanges: number; // Maximum video changes allowed before blocking
}
...
interface VideoChangeTracker {
  count: number;
  lastChangeTime: number;
}

// Track video changes per domain
const videoChanges: { [domain: string]: VideoChangeTracker } = {};

// Track tab audio status
const tabAudioStatus: { [tabId: number]: boolean } = {};

// Store for website categories and settings
let websiteCategories: WebsiteCategories = {
  fun: [],
  funAndWork: [],
  socialMedia: [],
};
          ...
          if (expiredDomains.has(domain)) {
            ...
            if (category) {
              const redirectUrl = chrome.runtime.getURL(
                `build/redirect.html?type=${category}&url=${encodeURIComponent(
                  tab.url
                )}`
              );
...
function checkWebsiteCategory(
  ...
  try {
    ...
    const domainMatches = (siteUrl: string, targetDomain: string): boolean => {
      try {
...
function isTemporarilyAllowed(url: string | undefined | null): boolean {
  ...
  try {
    ...
    const now = Date.now();
    return (
      Boolean(temporaryAllowances[domain]) &&
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === "loading" && tab.url) {
    ...
    if (isTemporarilyAllowed(tab.url)) return;
...
function safeGetDomain(url: string | undefined | null): string | null {
...
function addTemporaryAllowance(
  url: string,
  limitType: "time" | "video",
  purpose: string
) {
    ...
    if (category) {
      ...
      if (categorySettings) {
        ...
        if (videoChanges[domain]) {
          videoChanges[domain].count = 0;
          debugLog(
            `Reset video count for ${domain} after granting temporary access`
          );
        }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "VIDEO_CHANGED") {
    debugLog(
      "Received VIDEO_CHANGED message from tab " + sender.tab?.id,
      message
    );
    ...
    const url = message.url;
    const limitType = message.limitType as "time" | "video";
...
const handleVideoChangeDebounced = debounce((url: string, tabId: number) => {
  ...
  if (category && !isTemporarilyAllowed(url)) {
    ...
    const redirectUrl = chrome.runtime.getURL(
      `build/redirect.html?type=${category}&url=${encodeURIComponent(url)}`
    );
...
function getCategorySettings(category: string): TimerSettings | null {
  switch (category) {
    case "fun":
      return settings.fun;
    case "funAndWork":
...
function handleVideoChange(url: string, changeCount: number, videoInfo: any) {
  ...
  if (
    ...
    if (!isWatchPage) {
      debugLog(
        `Ignoring video change on non-watch/shorts YouTube page: ${url}`
      );
      return;
    }
  }
  ...
  debugLog(
    `Video count for ${domain}: ${previousCount} → ${videoChanges[domain].count} (from content: ${changeCount})`
  );
  ...
  let categorySettings: TimerSettings;
  ...
  if (videoChanges[domain].count > categorySettings.maxVideoChanges) {
    debugLog(
      `Excessive video changes detected for ${domain}: ${videoChanges[domain].count} (limit: ${categorySettings.maxVideoChanges})`
    );
